...
public:
  ...
  typedef mystl::reverse_iterator<iterator>        reverse_iterator;   //反转迭代器
  typedef mystl::reverse_iterator<const_iterator>  const_reverse_iterator;  //反转迭代器

  allocator_type get_allocator() { return data_allocator(); }
  //allocator_type get_allocator() { return data_allocator::allocate(); }
...
public:
  ...
  template <class Iter, typename std::enable_if<                 
    mystl::is_input_iterator<Iter>::value, int>::type = 0>  //区间构造函数
    ...
    cap_(rhs.cap_)
  {
    rhs.begin_ = nullptr;
    rhs.end_ = nullptr;
    rhs.cap_ = nullptr; //防止被析构
  }

  vector(std::initializer_list<value_type> ilist)  //initializer_list
  {
    range_init(ilist.begin(), ilist.end());
  }

  vector& operator=(const vector& rhs);     //重载 = 
  vector& operator=(vector&& rhs) noexcept;   //重载 = 

  vector& operator=(std::initializer_list<value_type> ilist)   //重载 =   两个vector 拷贝构造函数
  {
...
public:
  ...
  reverse_iterator       rbegin()        noexcept         //Iterator 中
  ...
  const_iterator         cbegin()  const noexcept           //（）重载？ begin函数
  { return begin(); }
  ...
  reference at(size_type n)    //at函数
  {
    THROW_OUT_OF_RANGE_IF(!(n < size()), "vector<T>::at() subscript out of range");
    return (*this)[n];
  }
  const_reference at(size_type n) const     //at函数
  {
    THROW_OUT_OF_RANGE_IF(!(n < size()), "vector<T>::at() subscript out of range");
    return (*this)[n];
  }
  ...
  void assign(size_type n, const value_type& value)         // assign  -----> fill_assign
  { fill_assign(n, value); }
  ...
  iterator insert(const_iterator pos, size_type n, const value_type& value)    // 群插入
  {
  ...
  {
    MYSTL_DEBUG(pos >= begin() && pos <= end() && !(last < first));    //区间拷贝函数
...
private:
  ...
  void      init_space(size_type size, size_type cap);     //初始空间

  void      fill_init(size_type n, const value_type& value);
  template <class Iter>
  void      range_init(Iter first, Iter last);     //区间初始化
...
vector<T>& vector<T>::operator=(const vector& rhs)
{
  if (this != &rhs)
  {
    ...
    else if (size() >= len)
    {
      ...
      data_allocator::destroy(i, end_);   //删除后面数据空间
      end_ = begin_ + len;                //end位置重新赋值
    }
    else
    { 
      mystl::copy(rhs.begin(), rhs.begin() + size(), begin_);
      mystl::uninitialized_copy(rhs.begin() + size(), rhs.end(), end_);   //底层是construct ---new 赋值操作
...
// 移动赋值操作符
template <class T>
vector<T>& vector<T>::operator=(vector&& rhs) noexcept     //容器原来迭代器 == nullptr
{
  destroy_and_recover(begin_, end_, cap_ - begin_);
  begin_ = rhs.begin_;
  end_ = rhs.end_;
...
vector<T>::emplace(const_iterator pos, Args&& ...args)
{
  ...
  if (end_ != cap_ && xpos == end_)  //插入元素在尾部
  {
    data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);
    ++end_;
  }
  else if (end_ != cap_)     //插入位置不在尾部
  {
    auto new_end = end_;
    data_allocator::construct(mystl::address_of(*end_), *(end_ - 1));
    ++new_end;
    mystl::copy_backward(xpos, end_ - 1, end_);    //从result位置向前拷贝
...
void vector<T>::emplace_back(Args&& ...args)
{
  if (end_ < cap_)
  {
    data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);   //construct(pos， value)
    ++end_;
  }
  else
  {
...
{
  MYSTL_DEBUG(!empty());    //先判断数组为空
  data_allocator::destroy(end_ - 1); //析构
  --end_;
}
...
vector<T>::erase(const_iterator pos)
{
  ...
  mystl::move(xpos + 1, end_, xpos); //往前覆盖
  data_allocator::destroy(end_ - 1); //destroy最后一个元素
  --end_;
  return xpos;
}
...
vector<T>::erase(const_iterator first, const_iterator last)
{
  ...
  iterator r = begin_ + (first - begin());  //r指向first位置
  data_allocator::destroy(mystl::move(r + (last - first), end_, r), end_);  //先移动，再destroy后面空间
...
void vector<T>::resize(size_type new_size, const value_type& value)
{
  if (new_size < size())
  {
	  ...
	  erase(begin() + new_size, end());   //如果小，则erase, erase底层就是destroy
  }
  else
  {
    insert(end(), new_size - size(), value);   //否则insert, insert底层是new
  }
}
...
void vector<T>::try_init() noexcept
{
  try
  {
    begin_ = data_allocator::allocate(16);    //16代表什么？？？
    end_ = begin_;
    cap_ = begin_ + 16;
  }
  catch (...)
...
// init_space 函数
template <class T>
void vector<T>::init_space(size_type size, size_type cap)    //初始化空间
{
  try
  {
...
fill_init(size_type n, const value_type& value)
{
  ...
  mystl::uninitialized_fill_n(begin_, n, value);   //从begin 填充n个value
}
...
template <class Iter>
void vector<T>::
range_init(Iter first, Iter last)//拷贝构造
{
...
get_new_cap(size_type add_size)
{
  ...
  THROW_LENGTH_ERROR_IF(old_size > max_size() - add_size,    //判断需要增加的大小
...
template <class T>
void vector<T>::
fill_assign(size_type n, const value_type& value)    //n个value赋值
{
  if (n > capacity())
  {
    vector tmp(n, value);
  ...
  {
    erase(mystl::fill_n(begin_, n, value), end_);   //先赋值再erase
  }
}
...
template <class IIter>
void vector<T>::
copy_assign(IIter first, IIter last, input_iterator_tag)      //拷贝赋值函数
{
  auto cur = begin_;
  ...
  if (first == last)    //当赋值结束，则destroy剩余空间
  {
    erase(cur, end_);
  }
  else                   // 空间用完，则insert剩余元素
  {
...
copy_assign(FIter first, FIter last, forward_iterator_tag)
{
  ...
  {
    auto mid = first;
    mystl::advance(mid, size());    //让iterator向前前进n个大小
...
{
  const auto new_size = get_new_cap(1);     //获得新的大小
  auto new_begin = data_allocator::allocate(new_size);
  auto new_end = new_begin;
  try
  {
    new_end = mystl::uninitialized_move(begin_, pos, new_begin);
    data_allocator::construct(mystl::address_of(*new_end), mystl::forward<Args>(args)...);   //address of 取地址
    ++new_end;
...
// 重新分配空间并在 pos 处插入元素                 //构造元素和插入元素的区别？？？？  forward和copy
template <class T>
