...
template <class Key, class T, class Compare = mystl::less<Key>>    // 1、键  2、值  3、比较方式
class map
{
public:
  // map 的嵌套型别定义
  ...
  typedef mystl::pair<const Key, T>  value_type;   //一个pair<键、值>
  typedef Compare                    key_compare;  //comp = less

  // 定义一个 functor，用来进行元素比较    
  //set没有！！！ map有！！！！！  set中 value_comp  = key_comp
...
public:
  ...
  map& operator=(map&& rhs)    //*this  map类型，  赋值tree_
  { 
    tree_ = mystl::move(rhs.tree_);
    return *this;
  }
  ...
  value_compare          value_comp()    const { return value_compare(tree_.key_comp()); }  //比较键值
  ...
  mapped_type& operator[](const key_type& key)  //重载[]
  {
    iterator it = lower_bound(key);
    if (it == end() || key_comp()(key, it->first))
      it = emplace_hint(it, key, T{}); //没有的话就插入，值默认是0
    return it->second;
  }
  mapped_type& operator[](key_type&& key)  //重载[]  右值引用
  {
    iterator it = lower_bound(key);
    if (it == end() || key_comp()(key, it->first))
