...
public:
  // 构造、复制、移动函数
  set() = default;   //调用编译器的默认构造函数， 无参

  template <class InputIterator>     //有参构造函数
  set(InputIterator first, InputIterator last) 
    :tree_() 
  { tree_.insert_unique(first, last); }

  set(std::initializer_list<value_type> ilist)   //有参构造函数， 一包参数。。。。
    :tree_()
  { tree_.insert_unique(ilist.begin(), ilist.end()); }

  set(const set& rhs)    //拷贝构造函数
    :tree_(rhs.tree_)
  {
  }

  set(set&& rhs) noexcept   //右引用拷贝构造函数
  ...
  set& operator=(set&& rhs)    //拷贝赋值 重载=
  { 
    tree_ = mystl::move(rhs.tree_); 
    return *this; 
  }
  set& operator=(std::initializer_list<value_type> ilist)   //拷贝赋值 重载=  一包
  {
    tree_.clear();
    tree_.insert_unique(ilist.begin(), ilist.end());
    return *this;
  ...
  key_compare      key_comp()      const { return tree_.key_comp(); }   //调用rb_tree的compare()
  value_compare    value_comp()    const { return tree_.key_comp(); }
  allocator_type   get_allocator() const { return tree_.get_allocator(); }   //调用rb_tree的allocate()

  // 迭代器相关

  iterator               begin()         noexcept     //调用tree的迭代器
  { return tree_.begin(); }
  ...
  // 容量相关
  bool                   empty()    const noexcept { return tree_.empty(); }    //调用tree函数
