...
private:
  container_type c_;  // 用底层容器表现 stack  //c_ 表示deque，默认的话就是deque为底层容器，具有vector和list的基本功能
...
public:
  ...
  stack(const Container& c)  //拷贝构造函数
    :c_(c)
  {
  }
  ...
  // 访问元素相关操作
  reference       top()       { return c_.back(); }  //top是deque的back
