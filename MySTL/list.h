...
template <class T>
struct node_traits                       //萃取机 ----萃取指针，头节点和节点
{
  typedef list_node_base<T>* base_ptr;  //本身就是节点类型
...
struct list_node_base                        
{
  ...
  base_ptr prev;  // 前一节点，指向自身类型的指针
  base_ptr next;  // 下一节点

  list_node_base() = default;  //默认构造函数

  node_ptr as_node()           
  {
    return static_cast<node_ptr>(self());    //把self()转换为node_ptr类型
  }

  void unlink()   
  {
    prev = next = self();   //只有一个节点情况
  }

  base_ptr self()               //头节点的自身
  {
    return static_cast<base_ptr>(&*this);        // 返回自身--base
  }
};

template <class T>       //节点结构
struct list_node : public list_node_base<T>    //继承base 指针和函数
{
  typedef typename node_traits<T>::base_ptr base_ptr;    //typedef
  typedef typename node_traits<T>::node_ptr node_ptr;

  T value;  // 数据域

  list_node() = default;
  list_node(const T& v) //赋值操作 value = v
    :value(v)
  {
  }
  list_node(T&& v) 
    :value(mystl::move(v))  //右值引用，用move，把左值改为右值
  {
  }

  base_ptr as_base()                 //是头节点---返回头节点
  {
    return static_cast<base_ptr>(&*this);   
  }
  node_ptr self()                   //是自身----返回自身---节点
  {
...
// list 的迭代器设计
template <class T>
struct list_iterator : public mystl::iterator<mystl::bidirectional_iterator_tag, T>  //继承双向迭代器类型
{
  ...
  base_ptr node_;  // 指向当前节点     //定义了指针指向当前节点

  // 构造函数
  list_iterator() = default;   //默认构造函数
  list_iterator(base_ptr x)    //有参构造函数
    :node_(x) {}
  list_iterator(node_ptr x)    //有参构造函数
    :node_(x->as_base()) {}
  list_iterator(const list_iterator& rhs)   //拷贝构造函数
    :node_(rhs.node_) {}

  // 重载操作符
  reference operator*()  const { return node_->as_node()->value; }  //重载* --解引用
  pointer   operator->() const { return &(operator*()); }    //重载  ->    ----取地址

  self& operator++()           // 重载int++ 返回自身
  {
    MYSTL_DEBUG(node_ != nullptr);  
    node_ = node_->next;
    return *this;    //*this指向的是迭代器
  }
  self operator++(int)   //重载++（int）
  {
    self tmp = *this;
    ++*this;
    return tmp;   //++int  重载
...
template <class T>
struct list_const_iterator : public iterator<bidirectional_iterator_tag, T>   //const 迭代器
{
  typedef T                                 value_type;
...
public:
  ...
  allocator_type get_allocator() { return node_allocator(); }  //分配空间函数
...
public:
  // 构造、复制、移动、析构函数
  list() 
  { fill_init(0, value_type()); }  //list内值赋值0

  explicit list(size_type n)       //list内值赋值0
  { fill_init(n, value_type()); }

  list(size_type n, const T& value)   //list内值赋值value
  { fill_init(n, value); }
  ...
  list& operator=(const list& rhs)   //拷贝构造函数=
  {
    if (this != &rhs)
    {
  ...
  list& operator=(list&& rhs) noexcept   //拷贝构造函数=  右引用
  {
    clear();
    splice(end(), rhs);
    return *this;
  ...
  list& operator=(std::initializer_list<T> ilist)  //拷贝构造函数=  右引用
  {
  ...
  ~list()  //析构函数
  {
    ...
    {
      clear();  //clear
      base_allocator::deallocate(node_);  //deallocate
      node_ = nullptr;  //赋值空指针
      size_ = 0;   //size = 0
    }
  }
...
public:
  // 迭代器相关操作
  iterator               begin()         noexcept  //node 指向尾节点
  { return node_->next; }
  ...
  size_type size()     const noexcept    //size()
  { return size_; }
  ...
  reference       back()   //back()是end()的前一个位置
  { 
    MYSTL_DEBUG(!empty());
    return *(--end());   
  }

  const_reference back()  const   //back()是end()的前一个位置
  { 
    MYSTL_DEBUG(!empty()); 
    return *(--end());
  }
  ...
  void     assign(size_type n, const value_type& value)    //assign n个value
  { fill_assign(n, value); }
  ...
  void     assign(std::initializer_list<T> ilist)   //打包赋值，initializer_list
  { copy_assign(ilist.begin(), ilist.end()); }

  // emplace_front / emplace_back / emplace

  template <class ...Args>          //插在链表头，无返回值
  ...
  template <class ...Args>       //插在链表尾，无返回值
  ...
  {
    THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
    auto link_node = create_node(value);  //调用构造函数
    link_nodes_at_front(link_node->as_base(), link_node->as_base());
    ++size_;
  }
  ...
  {
    MYSTL_DEBUG(!empty());
    auto n = node_->next;   //指向头节点
    unlink_nodes(n, n); // 断开头节点链接
...
{
  if (pos == node_->next)  //node是末尾节点，node->next是头节点
  {
    link_nodes_at_front(link_node, link_node);
  }
  else if (pos == node_)   //node是末尾节点，
  {
...
{
  iterator r(pos.node_);     //迭代器中也有node
  if (n != 0)
  {
    const auto add_size = n;
    auto node = create_node(value);
    node->prev = nullptr;  //新创建的Node的prev置空
    r = iterator(node);
    iterator end = r;
    try
    {
      ...
      for (--n; n > 0; --n, ++end)
      {
        ...
        end.node_->next = next->as_base();  // link node  ，  上一个创建的next指向新创建的
        next->prev = end.node_;  //新创建的prev指向上一个末尾
      }
      size_ += add_size;
    }
    catch (...)
...
// 在 pos 处插入 [first, last) 的元素    把value改成*first
template <class T>
template <class Iter>
typename list<T>::iterator 
