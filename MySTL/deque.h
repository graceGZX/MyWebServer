...
struct deque_iterator : public iterator<random_access_iterator_tag, T>
{
  ...
  static const size_type buffer_size = deque_buf_size<T>::value;  // 缓冲区大小

  // 迭代器所含成员数据
  value_pointer cur;    // 指向所在缓冲区的当前元素
  value_pointer first;  // 指向所在缓冲区的头部
  value_pointer last;   // 指向所在缓冲区的尾部
  map_pointer   node;   // 缓冲区所在节点    //明确node的作用！！！！
  ...
  {
	  return static_cast<difference_type>(buffer_size) * (node - x.node)  //距离计算 = buffer长度 *（buffer个数 - 1）+ 当前buffer中元素个数相减
  ...
  self& operator++()  
  {
    ++cur;
    if (cur == last)
    { // 如果到达缓冲区的尾---last的最后一个元素的下一个位置，所以要先++
      set_node(node + 1);
      cur = first;
    }
  ...
  self& operator+=(difference_type n) // +=  难点！
  {
	//const difference_type offset = n + (cur - first);  // auto 就是difference_type

	const auto offset = n + (cur - first);  //判断加上距离后是否超过了一个缓冲区大小
    ...
    { // 要跳到其他的缓冲区
      //const difference_type node_offset = offset > 0
	    ...
	    const auto node_offset = offset > 0   // node——offset是difference_type类型，代表node移动距离
        ? offset / static_cast<difference_type>(buffer_size)
        : -static_cast<difference_type>((-offset - 1) / buffer_size) - 1;
      set_node(node + node_offset); // node + int 节点前进
      cur = first + (offset - node_offset * static_cast<difference_type>(buffer_size)); //总移动个数-node移动数*buffer_size
    }
    return *this;
  }
...
public:
  ...
  reference       at(size_type n)      // 执行的什么操作？？？？
  { 
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
  if (this != &rhs)
  {
    ...
    if (len >= rhs.size())
    {
      erase(mystl::copy(rhs.begin_, rhs.end_, begin_), end_); //copy返回尾部
    }
    else
    {
      iterator mid = rhs.begin() + static_cast<difference_type>(len); //mid 指向 rhs中len 位置
      mystl::copy(rhs.begin_, mid, begin_); // 先把[begin,mid] copy
      insert(end_, mid, rhs.end_);  //insert(pos, first, last) ， 再把[len, end] insert
    }
  }
  return *this;
}
...
{
  clear();
  begin_ = mystl::move(rhs.begin_);  // 先move
  end_ = mystl::move(rhs.end_);
  map_ = rhs.map_;
  map_size_ = rhs.map_size_;
  rhs.map_ = nullptr;  //自身置空
  rhs.map_size_ = 0;
  return *this;
}
...
void deque<T>::resize(size_type new_size, const value_type& value)
{
  ...
  if (new_size < len)
  {
    erase(begin_ + new_size, end_); // 小于erase
  }
  else
  {
    insert(end_, new_size - len, value); // 大于insert
  }
}
...
{
  // 至少会留下头部缓冲区
  for (auto cur = map_; cur < begin_.node; ++cur)  //begin前的缓冲区都destroy
  {
    data_allocator::deallocate(*cur, buffer_size);
    *cur = nullptr;
  }
  for (auto cur = end_.node + 1; cur < map_ + map_size_; ++cur) // end 后面的缓冲区都destroy , 从end.node的后一个位置
  {
    data_allocator::deallocate(*cur, buffer_size);
    *cur = nullptr;
  }
...
{
  if (begin_.cur != begin_.first) // cur不在first
  {
    data_allocator::construct(begin_.cur - 1, mystl::forward<Args>(args)...); // cur 不是first,在cur前面构造
    --begin_.cur;
  }
  else  //cur 在first,在上一个缓冲区构造
  {
    require_capacity(1, true);
    try
    {
      --begin_; //迭代器减1，插入上一个缓冲区内
...
{
  if (end_.cur != end_.last - 1)  //cur 不在 end-1
  {
...
{
  if (pos.cur == begin_.cur) //判断在不在头
  {
    emplace_front(mystl::forward<Args>(args)...);
    return begin_;
  }
  else if (pos.cur == end_.cur) //判断在不在尾
  {
    emplace_back(mystl::forward<Args>(args)...);
    return end_ - 1;
  }
  return insert_aux(pos, mystl::forward<Args>(args)...); //insert_aux
}
...
{
  MYSTL_DEBUG(!empty());  //先判断是否为空
  if (begin_.cur != begin_.last - 1)
  {
    data_allocator::destroy(begin_.cur); //头不在尾部，直接destroy
    ++begin_.cur;
  }
  else
  {
    data_allocator::destroy(begin_.cur);  //头在尾部
    ++begin_;
    destroy_buffer(begin_.node - 1, begin_.node - 1); //destroy buffer
  }
}
...
{
  if (position.cur == begin_.cur)  //在头部用push_front
  {
    push_front(value);
    return begin_;
  }
  else if (position.cur == end_.cur)  //在尾部用push_back
  {
    push_back(value);
    auto tmp = end_;
    --tmp;
...
template <class T>
typename deque<T>::iterator
deque<T>::insert(iterator position, value_type&& value)  //右值引用用emplace_front/back
{
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
  if (position.cur == begin_.cur)
  {
    ...
    mystl::uninitialized_fill_n(new_begin, n, value); //初始化一段空间
    begin_ = new_begin;  //更新begin
  }
  else if (position.cur == end_.cur)
  {
...
deque<T>::erase(iterator position)
{
  ...
  {
    mystl::copy_backward(begin_, position, next);    //  将 [first, last)区间内的元素拷贝到 [result - (last - first), result)内 ,相当于position全部后退一位
    pop_front(); // 弹出头元素
  }
  else
  {
...
deque<T>::erase(iterator first, iterator last)
{
  ...
  else
  {
    ...
    {
      mystl::copy_backward(begin_, first, last);  //前面少 往后移
...
void deque<T>::clear()
{
  ...
  shrink_to_fit();  //map_ 前后都收缩，只保留头部缓冲区
  end_ = begin_;
}
...
fill_init(size_type n, const value_type& value)
{
  ...
  if (n != 0)
  {
    ...
    {
      mystl::uninitialized_fill(*cur, *cur + buffer_size, value); //每一个缓冲区从头到尾 init
    }
...
fill_assign(size_type n, const value_type& value)
{
  ...
  {
    erase(begin() + n, end()); //erase 更新了end
    mystl::fill(begin(), end(), value);
  }
}
...
copy_assign(IIter first, IIter last, input_iterator_tag)
{
  ...
  {
   insert_dispatch(end_, first, last, input_iterator_tag{}); //  创造容量，在end_位置一直插入last值， last--
  }
}
...
{  
  ...
  {
    auto next = first;
    mystl::advance(next, len1); // 让next前进len1
    mystl::copy(first, next, begin_);  //begin后拷贝
    insert_dispatch(end_, next, last, forward_iterator_tag{}); //在end_位置一直插入last值， last--
  }
  else
  {
...
insert_aux(iterator position, Args&& ...args)
{
  ...
  if (elems_before < (size() / 2))  //前面值少， 把pos前面的值在 begin + elems_before位置 copy
  { // 在前半段插入
    emplace_front(front());
    auto front1 = begin_;
    ++front1;
    auto front2 = front1; //front2 在front1 后面
    ++front2;
  ...
  { // 在后半段插入                           //后面值少， 把pos后面的值在begin + elems_before位置 copy
    emplace_back(back());
    auto back1 = end_;
    --back1;
    auto back2 = back1;
...
insert_dispatch(iterator position, IIter first, IIter last, input_iterator_tag)
{
  ...
  const size_type elems_before = position - begin_;  //计算位置
  if (elems_before < (size() / 2))
  {
    require_capacity(n, true);
  }
